...
def assess_seller_card_merit(row: dict) -> dict:
    ...
    # A famous player plus the word rookie is not price evidence. With no exact
    # SOLD and no verified premium variant, keep it available for research but
    # prevent collector hype from dominating Seller Top 5.
    ...
    eligible=integrity["eligible_physical_single_card"] and team_badge_evidence_ok and (
        ...
        or (score>=25 and not mass_market_base and not faux_premium and not generic_rookie)
    ...
    return {"score":round(score,1),"eligible":bool(eligible),"mass_market_base":mass_market_base,"mass_market_team_badge":mass_market_team_badge,"faux_premium":faux_premium,"generic_rookie_without_sales":generic_rookie,"strong_signals":strong,"integrity":integrity,"reasons":reasons}
