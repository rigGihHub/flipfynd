...
def research_identity_failure_diagnostics(items):
    ...
    for item in items or []:
        if item.get("exact_identity_gate_supports_comp_research") or item.get("exact_identity_gate_status") == "SÖKBAR_TITEL":
            continue
        locked += 1
        missing = list(item.get("exact_identity_gate_research_missing_fields") or item.get("exact_identity_gate_missing_fields") or [])
        touched = False
        for field in missing:
            ...
            if key in counts:
                counts[key] += 1
                touched = True
        blockers = " ".join(str(x).casefold() for x in (item.get("exact_identity_gate_blockers") or []))
        if "motstridig" in blockers or "lot/multipack" in blockers:
            counts["konflikt/lot"] += 1
            touched = True
        if not touched:
            counts["övrigt"] += 1
    return {
